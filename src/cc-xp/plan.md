...
2. **story 完了時**
   **重要**: 完了したストーリーの内容を必ず分析し、以下の基準に基づいて次のステップを自動判定してください。

   **判定手順**:
   1. docs/cc-xp/stories/[ID].mdの内容を確認
   2. 実装に必要な技術要素を抽出・分析
   3. 以下の基準と照合して適切な次のステップを判定
   ...
   **→ /cc-xp:develop を案内する場合（即座に開発可能）**:
   - 既存技術スタックで実装可能な基本機能
   - 標準的なCRUD操作やUI要素
   - 類似実装が既存コードベースに存在
   - チームが熟知している技術での実装

   **判定実行**: ストーリー内容を分析後、該当する条件を明示して次のステップを案内してください。
