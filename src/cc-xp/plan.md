...
**minimum_experience** から **acceptance_criteria** を必ず導出してください：
...
stories:
  - id: [story-id]
    ...
    # 🎯 新規必須項目：受け入れ基準（E2Eテスト直接変換用）
    acceptance_criteria:
      - "アプリケーション起動/ページ表示の具体的な確認項目"
      - "出力要素（画面、UI、音声等）の存在と動作確認"
      - "時間経過・操作による変化の確認"
      - "プレースホルダーメッセージの不在確認"
      - "エラー処理の適切性確認"
      - "minimum_experienceの完全実現確認"
...
acceptance_criteria:
  ...
  - "ゲーム開始と同時にテトロミノが画面上部に出現する"
  - "テトロミノが1秒間隔で自動的に1行下に移動する"
  - "ゲームフィールドの境界が視覚的に明確に識別できる"
