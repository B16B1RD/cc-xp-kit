...
**技術調査が必要かどうかを判定し、適切な案内を表示してください**：

#### 技術調査の必要性判定

以下の基準で判定してください：

**技術調査が必要（/cc-xp:research推奨）**:
- 未知のライブラリ・フレームワークの使用
- 複雑なアルゴリズムの実装
- 公式仕様の確認が必要
- 外部API仕様の調査が必要
- セキュリティ実装のベストプラクティス確認

**技術調査が不要（/cc-xp:develop直行）**:
- 既知技術の単純な組み合わせ
- 明らかな実装パターン
- プロトタイプ・PoC的実装
...
**→ /cc-xp:research を推奨する場合（技術調査が必要）**:
```
🔬 次のステップ
...
Red状態のテストを Green にするため、[機能名]の実装を開始してください。
```

**デフォルト**: 迷った場合は /cc-xp:research を推奨（正確な実装のため）
