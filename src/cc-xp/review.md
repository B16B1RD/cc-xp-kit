...
- **引数がない場合**:
  ...
  - 評価結果表示後、以下のガイダンスを表示して終了：
    ...
    サーバーが停止中の場合：
    1. npx http-server . -p 8080 -c-1 & を実行  
    2. <http://localhost:8080> をブラウザで開く
    3. 実際の動作と価値体験を確認
    
    ✅ 確認必須ポイント：
    - プレースホルダーメッセージが表示されていない
    - minimum_experienceが実際に体験可能  
    - acceptance_criteriaの機能が動作している
    
    📊 **成果物確認後、以下のコマンドで判定してください：**
...
E2Eテストを30秒のタイムアウト付きで実行してください：

```bash
# E2Eテスト実行（30秒タイムアウト付き）
timeout 30s npm run test:e2e || timeout 30s npx playwright test || echo "E2Eテストタイムアウト"
```

**E2Eテスト実行の改善されたハンドリング**:
- E2Eテストが30秒以内に完了しない場合は、タイムアウトとして処理
- タイムアウトまたは失敗した場合は、手動での動作確認に切り替え
- 長時間実行が続く場合は、プロセスを強制終了して続行
...
サーバーが停止中の場合は、以下の手順で成果物を確認してください：
...
2. **確認URL**
   ...
   - 指定されたindex.htmlまたはメインページをブラウザで開く

3. **確認すべきポイント**
   - ✅ プレースホルダーメッセージ（「開発中」「準備完了」等）が表示されていない
   - ✅ backlog.yamlのminimum_experienceが実際に体験可能
   - ✅ acceptance_criteriaで定義された機能が動作している
   - ✅ ユーザーが期待通りの価値体験を得られる

**重要**: 評価判定の前に、必ず上記URLで実際の成果物を確認してください。
...
評価結果表示前に、必ず以下を強調してユーザーに案内してください：
