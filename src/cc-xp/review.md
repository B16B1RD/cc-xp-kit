...
@docs/cc-xp/backlog.yaml から `testing` ステータスのストーリーを確認し、**仮説検証に必要な全情報**を取得してください：
...
- **hypothesis_status**: 実装状況（implemented/pending/failed）
- **kpi_measurements**: develop.mdで記録された実測値
...
## Phase 3: 仮説検証重視の動作確認ガイド

**ストーリーの仮説とKPI測定結果**を基に、ビジネス価値実現を重点的に確認してください：
...
📊 KPI実績確認：
□ [metric1]: 目標[target1] → 実績[actual1] ([達成/未達成])
□ [metric2]: 目標[target2] → 実績[actual2] ([達成/未達成])
□ [metric3]: 目標[target3] → 実績[actual3] ([達成/未達成])

🏆 ビジネス価値実現確認：
□ [シナリオ1: 核心仮説検証] - KPI達成度[X%]
□ [シナリオ2: ユーザー体験検証] - ペルソナ適合度
□ [シナリオ3: 競合差別化確認] - 優位性実現度

🎮 実体験による確認：
1. [user_personaの視点で操作] - 「忙しいビジネスパーソンとして3秒以内でゲーム開始」
2. [kpi_targetの測定] - 「実際の起動時間をストップウォッチで計測」
3. [competition_analysisの検証] - 「他のテトリスゲームと比較して優位性を確認」

💡 開発者メモ:
[development_notesの内容を表示]
```
...
**仮説駆動の確認**（価値中心）：
- 忙しいビジネスパーソンが本当に3秒以内でゲームを開始できるか？
- 5-10分の短時間で満足できる体験になっているか？  
- 他のテトリスゲームより手軽で魅力的か？
