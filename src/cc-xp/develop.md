...
- **Canvas/DOM描画処理**: ユーザーが視覚的に確認できる要素の実装
- **基本的なレンダリング**: データ変更が画面に反映される仕組み  
- **初期表示**: 開発メッセージではなく実際のゲーム要素の表示
- **価値体験の最小実装**: core_value が実際に体験できる状態

**❌ 「最小限」の誤解例**：
- ロジック（データの変更）のみでテストをPASS  
- 視覚的要素を「追加機能」として後回し
- 「TDD実装準備完了」等の開発メッセージのまま終了

**✅ 正しい「最小限」**：
- ユーザーが minimum_experience を実際に体験できる最小実装
- テストPASS + 価値体験実現の両立
- 技術的成功と価値実現の統合
