...
```bash
# 自動修正ツールが標準MVPパターンを適用
node ~/.claude/commands/shared/story-auto-fix.js
```

**標準修正パターン**:
- **ハードドロップ機能**: 現代テトリスの基本操作として必須
- **レベル・速度システム**: ゲームの緊張感・やりがいの源泉  
- **7-bag randomizer**: 公平な出現システム
- **SRS回転システム**: 操作性・品質の現代標準

##### 3. 受け入れ基準の現実化

```markdown
**追加される受け入れ基準**:
- [ ] GIVEN 5分間プレイ WHEN 集中してプレイ THEN 適度な緊張感と達成感が得られる
- [ ] GIVEN ハードドロップ使用 WHEN スペース押下 THEN 瞬間的にピース配置できる  
- [ ] GIVEN レベル上昇 WHEN 時間経過 THEN 落下速度が段階的に上昇する
```
...
```bash
# MVP検証ゲート循環実行（最大3回試行）
bash ~/.claude/commands/shared/mvp-gate-loop.sh
```

**循環プロセス**:
1. 修正適用 → 2. 再検証 → 3. PASS時Phase 4進行 / FAIL時次の試行

##### 5. 修正内容の自動記録

全ての修正内容は以下に自動記録されます：
- **user-stories-v1.0.md**: 修正された機能・基準
- **バックアップファイル**: 修正前の状態を保持
- **学習記録**: 修正プロセスの詳細
...
**自動復旧の流れ**:
1. **問題の特定**: MVP検証で欠落要素を自動検出
2. **修正案の適用**: 標準的なMVP修正パターンを自動適用
   - ハードドロップ機能追加
   - レベル・速度システム追加  
   - SRS回転システム追加
   - 現実的な受け入れ基準に修正
3. **ストーリー更新**: user-stories-v1.0.mdを自動更新
4. **再検証実行**: 修正版で再度Phase 3.6を実行
5. **合格後自動進行**: 合格時はPhase 4に自動進行

##### 🛠️ 使用される自動修正ツール

**story-auto-fix.js**: 修正案に基づくストーリー自動更新
```bash
node ~/.claude/commands/shared/story-auto-fix.js [stories-file]
```

**mvp-gate-loop.sh**: 検証→修正→再検証の循環管理
```bash
bash ~/.claude/commands/shared/mvp-gate-loop.sh [stories-file] [max-retries]
```
...
3回の自動復旧が失敗した場合のみ、以下を手動実行：

1. **根本的な要件見直し**: プロジェクト規模・目標の再評価
2. **MVP定義の修正**: より現実的な最小版の定義
3. **技術選択の見直し**: 制約との適合性確認
4. **専門家への相談**: ドメインエキスパートとの相談

##### 🎯 自動復旧の利点

- **即座の問題解決**: 手動待機時間ゼロ
- **一貫した修正品質**: 標準的なMVPパターンを適用
- **学習機会の確保**: 修正内容の自動記録
- **勇気の実践**: 自動的な計画修正の実行

### 検証実行の完全自動化

この検証は**完全自動化**されており、以下の利点があります：

✅ **自動循環**: 問題発見→修正→再検証を自動実行
✅ **即座実行**: 手動介入なしでの問題解決
✅ **品質保証**: 標準的なMVP品質基準の自動適用
✅ **学習記録**: 修正プロセスの自動記録
...
**Kent Beck**: "勇気を持って現実を受け入れ、必要な変更を恐れるな" - **今は自動実装**
...
以下を実行して、プロジェクトタイプに応じた**具体的な完了メッセージ**を表示してください：
...
1. **プロジェクトタイプ自動判定**: 体験重視型/開発効率型/CLI型/API型
2. **プロジェクト情報抽出**: user-storiesからプロジェクト名・出力ファイル・期待動作を取得
3. **適切なテンプレート選択**: タイプに応じた具体的なテスト手順を生成
4. **完了メッセージ表示**: ユーザーが迷わない明確な次ステップ案内

**表示される内容例**：
- ✅ **具体的なファイル名とテスト方法**（"dist/tetris.html をダブルクリック"）
- ✅ **期待される動作の詳細説明**（ユーザーストーリーから自動抽出）
- ✅ **簡単なフィードバック質問**（はい/いいえで回答）
- ✅ **明確な次ステップ**（満足時は完了、継続時は /tdd:run）

**重要**: この汎用システムにより、**ゲーム・Webアプリ・CLI・API**等、どのプロジェクトタイプでもユーザーが迷わずテスト・フィードバックできるようになります。

**Kent Beck**: "完了の定義は、ユーザーが価値を実感できること"