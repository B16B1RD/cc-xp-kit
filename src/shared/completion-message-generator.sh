...
replace_template_variables() {
    ...
    sed -i "s|#{START_COMMAND}|$START_COMMAND|g" "$temp_file"
    ...
    # 複雑な変数（改行を含む）をawk で置換
    awk -v behaviors="$EXPECTED_BEHAVIORS" -v progress="$STORY_PROGRESS_SUMMARY" '
        { gsub(/#{EXPECTED_BEHAVIORS}/, behaviors) }
        { gsub(/#{STORY_PROGRESS_SUMMARY}/, progress) }
        { print }
    ' "$temp_file"
...
main() {
    ...
    # プロジェクト情報を抽出
    extract_project_info "$STORIES_FILE"
