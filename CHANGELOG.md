...
#### XP Kit エージェント・コマンド統合改善

**エージェントとコマンドの責務明確化と効率化**

- **intake-classifier**: 130 行削減、責務の簡潔化
- **discovery**: 100 行→簡潔化、Intent Model 生成に特化
- **preview/previewer**: preview-plan.yaml 追加、プレビュー機能強化
- **devops**: CI/CD 設定の詳細化と自動化強化
- **全体的なコードの簡潔化**: 493 行削減、442 行追加

#### 変更ファイル

- **8エージェント**: 責務明確化と冗長性削減
- **8コマンド**: ワークフロー最適化
- **2テンプレート**: quality-gates, release-strategy 改善
- **新規**: preview-plan.yaml テンプレート追加

#### 開発環境改善

- **package.json追加**: markdownlint/textlint 環境構築
- **.markdownlintignore追加**: node_modules/を除外
- **textlint設定**: スペーシング修正とルール適用
- **GitHub Actionsテスト修正**: v0.3.2 アーキテクチャ対応

#### テクニカル

- **コード品質**: textlint ルール準拠、Markdownlint 対応
- **テスト修正**: 新ファイル構造に対応したキーワードチェック
- **依存関係**: markdownlint-cli, textlint 関連パッケージ追加
...
- **discovery-intent.yaml** - Intent Model 構造化テンプレート
- **architecture.md** - C4 アーキテクチャ設計テンプレート
- **adr.md** - Architecture Decision Record テンプレート
- **acceptance_criteria.feature** - Gherkin 形式受け入れ条件
...
**新しいXPコマンド体系**
- `/xp:discovery "<曖昧要件>"` - Intent Model による要件構造化
- `/xp:design` - C4 設計・ADR・API 仕様の自動生成
- `/xp:scaffold` - 最小限足場の自動構築
- `/xp:tdd "<story>"` - Red→Green→Refactor TDD 実装
- `/xp:cicd` - CI/CD パイプライン・品質ゲート設定
...
- 文脈独立管理による専門性の向上
- intake-classifier, architect, scaffolder など 7 つのエージェント

**Intent Model 駆動設計**
- 曖昧要件からの capabilities 信頼度分析
...
**コマンド体系の変更**
- 旧: `/cc-xp:plan|story|develop|review|retro` (5 コマンド)
- 新: `/xp:discovery|design|scaffold|tdd|cicd|review|doc|retro|preview` (9 コマンド)
...
**設計アプローチの変更**
- 旧: フィーチャーレベル TDD 粒度システム
- 新: Intent Model 駆動 + MVP/Add-ons 分離
...
- **Capability Analysis**: 信頼度付き feature 候補分析
